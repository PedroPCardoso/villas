      ...
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-gray-100 bg-white/95 backdrop-blur-md px-6 sm:px-10 py-3">
        ...
        <div className="hidden lg:flex flex-1 justify-end gap-8 items-center">
          <nav className="flex items-center gap-9">
            ...
            <Link href="/catalogo" className="text-sm font-medium hover:text-primary transition-colors">Frota</Link>
            <button className="text-sm font-medium hover:text-primary transition-colors">Sobre nós</button>
          </nav>
          <div className="flex gap-3">
            <Link href="/cliente" className="px-4 py-2 text-sm font-bold border border-gray-200 rounded-xl hover:bg-gray-50 flex items-center h-10 transition-colors">Login</Link>
            <Link href="/admin" className="px-4 py-2 text-sm font-bold bg-primary text-white rounded-xl hover:bg-primary-hover shadow-sm shadow-red-500/20 flex items-center h-10 transition-all">Dashboard Admin</Link>
          </div>
        </div>
      </header>
        ...
        <div className="flex flex-col-reverse lg:flex-row items-center gap-12 lg:gap-20">
          <div className="flex flex-col gap-8 flex-1">
            ...
            <div className="flex flex-wrap gap-4">
              <Link href="/cliente/vistoria" className="min-w-[160px] bg-primary text-white font-bold h-12 px-6 rounded-xl hover:bg-primary-hover shadow-lg shadow-red-500/30 transition-all hover:-translate-y-0.5 flex items-center justify-center">
                Agendar Vistoria
              </Link>
              <Link href="/catalogo" className="min-w-[160px] bg-white border border-gray-200 text-slate-900 font-bold h-12 px-6 rounded-xl hover:bg-gray-50 flex items-center justify-center transition-colors">
                Ver Veículos
              </Link>
            </div>
          </div>
      ...
      <section className="bg-gray-50 py-20 px-6 sm:px-10">
        <div className="max-w-7xl mx-auto flex flex-col gap-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { title: 'Vistoria Offline', icon: 'wifi_off', text: 'Realize inspeções mesmo sem internet.' },
              { title: 'Alertas de Manutenção', icon: 'build_circle', text: 'Notificações automáticas de revisões.' },
              { title: 'Proteção Jurídica', icon: 'gavel', text: 'Contratos digitais com validade jurídica.' },
            ].map((f, i) => (
      ...
      <footer className="bg-primary text-white py-16 px-6 sm:px-10 mt-auto">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
          <div className="col-span-1 md:col-span-2">
            <h2 className="text-2xl font-black uppercase mb-4 font-logo">Villas Locações</h2>
