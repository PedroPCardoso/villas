...
import { INSPECTIONS } from '../constants';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f6f6f8]">
      {/* Mobile Action Bar */}
      <div className="md:hidden p-4 bg-white border-b border-gray-200 sticky top-16 z-10 space-y-3">
        <div className="relative">
          <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">search</span>
          <input className="pl-10 pr-4 py-2 bg-slate-100 border-none rounded-lg text-sm w-full focus:ring-2 focus:ring-primary" placeholder="Buscar veículo..." />
        </div>
        <button className="w-full bg-primary text-white py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 hover:bg-primary-hover shadow-sm">
      ...
      <header className="hidden md:flex h-16 px-8 items-center justify-between bg-white border-b border-gray-200 sticky top-0 z-10">
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">search</span>
            <input className="pl-10 pr-4 py-2 bg-slate-100 border-none rounded-lg text-sm w-64 focus:ring-2 focus:ring-primary" placeholder="Buscar veículo..." />
          </div>
      ...
      <div className="p-4 md:p-8 space-y-6 md:space-y-8 max-w-[1400px] mx-auto w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              ...
              <select className="text-xs border-gray-200 rounded-lg">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex justify-between items-center">
            ...
            <button onClick={() => onNavigate('FLEET')} className="text-sm font-bold text-primary hover:underline">Ver tudo</button>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50 text-[10px] uppercase font-bold text-slate-400 tracking-wider">
                  ...
                  <th className="px-4 md:px-6 py-4 hidden sm:table-cell">Inspetor</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {INSPECTIONS.map((ins, i) => (
                  <tr key={i} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-4 md:px-6 py-4 text-sm text-slate-500 hidden sm:table-cell">{ins.inspector}</td>
