...
const VehicleCatalog: React.FC<VehicleCatalogProps> = ({ onNavigate }) => {
  return (
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-gray-200 bg-white/80 backdrop-blur-md px-6 md:px-10 py-4">
        <div onClick={() => onNavigate('LANDING')} className="flex items-center gap-3 cursor-pointer">
          ...
          <div className="flex flex-col leading-none">
            <span className="text-2xl font-black tracking-tighter uppercase text-primary font-logo">Villas</span>
            <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-slate-600">Locações</span>
          </div>
        </div>
        <div className="flex gap-3">
          <button onClick={() => onNavigate('CLIENT_DASHBOARD')} className="flex h-10 items-center px-4 text-sm font-bold border border-gray-200 rounded-lg hover:bg-gray-50">Entrar</button>
          <button className="flex h-10 items-center px-4 text-sm font-bold bg-primary text-white rounded-lg shadow-lg shadow-red-900/20">Cadastrar</button>
        </div>
      </header>

      <main className="flex-1 max-w-[1440px] mx-auto px-6 py-12 md:px-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-10">
          <div className="max-w-2xl">
            <h1 className="text-3xl md:text-4xl font-black text-slate-900 leading-tight mb-4">Encontre o veículo ideal para sua jornada</h1>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-8 sticky top-20 z-40">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input className="w-full rounded-xl border-gray-200 bg-gray-50 py-3 pl-12 pr-4 text-sm outline-none focus:border-primary" placeholder="Buscar por modelo, marca..." />
            </div>
          ...
          {VEHICLES.concat(VEHICLES).map((v, i) => (
            <div key={i} className="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col">
              <div className="relative aspect-[4/3] bg-gray-100 overflow-hidden">
                <img src={v.image} className="w-full h-full object-cover transition-transform group-hover:scale-105 duration-500" alt={v.name} />
